...
def load_toml(config_path: str) -> dict[str, object]:
...
def split_model(model_str: str | None) -> tuple[str | None, str | None]:
    if not model_str:
        return None, None
    if ":" not in model_str:
...
def unsafe_default() -> list[str]:
    return ["rm", "rmdir", "del", "erase", "remove-item", "format", "mkfs", "shutdown"]
...
def load_settings() -> dict[str, object]:
    ...
    model_provider_from_str, model_from_str = split_model(model)
    ...
    compaction_enabled = parse_bool(compaction_cfg.get("enabled", True))
    trigger_ratio = float(compaction_cfg.get("trigger_ratio") or 0.8)
    prune_outputs = parse_bool(compaction_cfg.get("prune_tool_outputs", True))
    tool_output_keep = int(compaction_cfg.get("tool_output_keep") or 10)
    ...
    return {
        ...
        "hook_pre": listify(hooks_cfg.get("pre") or []),
        "hook_post": listify(hooks_cfg.get("post") or []),
